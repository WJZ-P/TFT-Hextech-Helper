 ...
 * 使用方式：
 * ```typescript
 * const emitter = GameStageEmitter.getInstance();
 * emitter.on('stageChange', (event) => { ... });
 * emitter.start();
 * ```
 */
...
/**
 * 事件类型映射
 * @description 定义 GameStageEmitter 支持的所有事件及其数据类型
 */
 ...
 */
export class GameStageEmitter extends EventEmitter {
    private static instance: GameStageEmitter;
     ...
     * 获取单例实例
     */
    public static getInstance(): GameStageEmitter {
        if (!GameStageEmitter.instance) {
            GameStageEmitter.instance = new GameStageEmitter();
        }
        return GameStageEmitter.instance;
    }
    ...
    public start(interval: number = 1000): void {
        if (this.isRunning) {
            logger.warn('[GameStageEmitter] 已经在运行中，忽略重复启动');
            return;
        }
        ...
        logger.info(`[GameStageEmitter] 启动阶段轮询，间隔: ${interval}ms`);
    ...
    public stop(): void {
        if (!this.isRunning) {
            logger.debug('[GameStageEmitter] 未在运行，忽略停止请求');
            return;
        }
        ...
        logger.info('[GameStageEmitter] 阶段轮询已停止');
    }
    ...
    public reset(): void {
        ...
        logger.info('[GameStageEmitter] 状态已重置');
    }
        ...
        try {
            ...
            if (stageText !== this.lastStageText) {
                ...
                if (parsed) {
                    ...
                    // 判断是否进入新的大阶段
                    const isNewStage = stage !== this.lastStage;
                    ...
                    // 发出事件
                    logger.info(
                        `[GameStageEmitter] 阶段变化: ${stageText} ` +
            ...
            logger.error(`[GameStageEmitter] 阶段检测异常: ${error}`);
        }
    }
    ...
    private parseStageText(stageText: string): { stage: number; round: number } | null {
        ...
        if (!match) {
            ...
            logger.debug(`[GameStageEmitter] 无法解析阶段文本: "${stageText}"`);
            return null;
        }
    ...
    private async checkFightingPhase(): Promise<void> {
        ...
        //     logger.info('[GameStageEmitter] 检测到"战斗环节"，进入战斗状态');
...
/** GameStageEmitter 单例实例 */
export const gameStageEmitter = GameStageEmitter.getInstance();
